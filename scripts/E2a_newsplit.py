...
class SpineBenchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.exists(img_path):
            img_path = os.path.join(self.img_root, os.path.basename(sample["image"]))
        if not os.path.exists(img_path):
            img_path = os.path.join(cfg.DATA_ROOT, sample["image"])
        if not os.path.exists(img_path):
            img_path = os.path.join(cfg.DATA_ROOT, "all", sample["image"])
        if not os.path.exists(img_path):
            img_path = os.path.join(cfg.DATA_ROOT, "evaluation", sample["image"])
...
def main():
    wandb.init(
        project="SpineVQA-CL",
        name="E2a-DiseaseContrastive-NewSplit",
        config={
            "lr":                 cfg.LR,
    ...
    val_dataset = SpineBenchDataset(
        cfg.VAL_JSON, cfg.VAL_IMG_ROOT,
        processor, tokenizer, split="val"
    )
    ...
    best_epoch = 0
    best_path = os.path.join(cfg.SAVE_DIR, "E2a_best.pth")
    print("\nStarting training...")
    print("=" * 80)
    print(f"{'Epoch':>6} | {'Loss':>8} | {'Cls Acc':>8} | "
          f"{'Loc Acc':>8} | {'Pre':>7} | {'Rec':>7} | "
          f"{'ValOverall':>10} | {'LR':>10}")
    print("-" * 80)
    ...
    for epoch in range(1, cfg.EPOCHS + 1):
        ...
        wandb.log({
            ...
            "val_loc_exact_acc": metrics["loc_exact_acc"],
            "precision":     metrics["precision"],
            "recall":        metrics["recall"],
            "val_overall_acc": metrics["overall_acc"],
            "lr":            current_lr,
        })

        # Save best
        if metrics["overall_acc"] > best_val_overall:
            best_val_overall = metrics["overall_acc"]
            best_epoch = epoch
