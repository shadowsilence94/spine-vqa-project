# ============================================================
# E7a — HVA-Net: Hierarchical Vertebra-Aware Cross-Attention
# ============================================================
# Novel Architecture (our own invention):
#
# Key idea: Disease and location are naturally correlated.
# Instead of predicting them separately (E4b),
# we model their JOINT relationship via a
# Disease-Location Evidence Matrix.
#
# Novel Modules:
#   1. VertebralAttention  → 5 level features [B,5,768]
#   2. DiseaseQueryAttention → 12 disease features [B,12,768]
#   3. EvidenceMatrix → joint [B,12,5] scoring
#
# Prediction:
#   Disease = pool evidence over 5 levels → [B,12]
#   Location = pool evidence over 12 diseases → [B,5]
#
# Loss:
#   L_total = L_disease + L_loc + λ * L_joint
#   L_joint = BCE on 12×5 evidence matrix
#             (forces model to learn disease-level binding)
#
# Expected improvement over E4b (58.32%):
#   Joint reasoning should improve both tasks!
# ============================================================
...
class SpineBenchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Build joint target matrix [12, 5]
        # = 1 only at (disease_idx, level_idx) for paired samples
...
# ============================================================
# 3. HVA-NET ARCHITECTURE
# ============================================================

class VertebralAttention(nn.Module):
    """
    Module 1: Vertebral Attention (same as E4b)
    5 learnable level queries attend to 196 patch tokens
    → [B, 5, 768] level-specific features
    """
...
class DiseaseQueryAttention(nn.Module):
    """
    Module 2: Disease Query Attention (NEW!)
    12 learnable disease queries attend to vertebral features
    → [B, 12, 768] disease-specific features

    Each query learns to extract evidence for one disease:
    Query 0 → Subarticular Stenosis visual evidence
    Query 1 → Foraminal Stenosis visual evidence
    ...etc
    """
    def __init__(self, dim=768, num_diseases=12, num_heads=8):
        super().__init__()
        # 12 learnable disease queries
    ...
    def forward(self, level_feats, q_feat=None):
        """
        level_feats: [B, 5, 768]
        q_feat:      [B, 768] optional question conditioning

        Returns: disease_feats [B, 12, 768]
        """
        ...
        # Optional: condition disease queries on question
        if q_feat is not None:
            q_proj = q_feat.unsqueeze(1).expand(-1, cfg.NUM_DISEASE, -1)
            queries = queries + 0.1 * q_proj

        # Each disease query attends to 5 vertebral features
        disease_feats, _ = self.attn(
            queries,     # [B, 12, 768] query
            level_feats, # [B, 5, 768]  key
            level_feats  # [B, 5, 768]  value
        )
        return self.norm(disease_feats)  # [B, 12, 768]


class HVANet(nn.Module):
    """
    HVA-Net: Hierarchical Vertebra-Aware Cross-Attention Network

    Novel architecture for Spinal Pathology VQA.

    Key contribution: Joint Disease-Location Evidence Matrix
    Instead of separate disease/location heads,
    we compute a 12×5 evidence matrix and pool from it.

    Architecture:
        Image → SigLIP2 → [B,196,768]
             → VertebralAttention → [B,5,768]
             → DiseaseQueryAttention → [B,12,768]
             → EvidenceMatrix → [B,12,5]
             → Disease Head (pool over levels)
             → Location Head (pool over diseases)
    ...
    def __init__(self):
        ...
        # BERT — Frozen (focus on HVA modules only)
        self.bert = BertModel.from_pretrained(cfg.BERT_NAME)
        for p in self.bert.parameters():
            p.requires_grad = False
        print("BERT: FROZEN ❄️")

        # Module 1: Vertebral Attention (from E4b)
    ...
    def forward(self, images, input_ids, attention_mask):
        ...
        Returns:
            disease_logits: [B, 12]
            loc_logits:     [B, 5]
            evidence_matrix:[B, 12, 5]  ← for joint loss
            vert_attn:      [B, 5, 196] ← for visualization
        """
        # ── SigLIP2: Extract patch tokens ─────────────
        with torch.no_grad():
            vision_out   = self.siglip2.vision_model(
                pixel_values=images
            )
            patch_tokens = vision_out.last_hidden_state
            # [B, 196, 768]
        ...
        # Fix 2: logit_scale makes values usable as logits
        evidence_matrix = self.logit_scale * torch.bmm(
            d_norm, l_norm.transpose(1, 2)
        )  # [B, 12, 5]

        # Fix 1: True HVA-Net prediction from evidence matrix
        # Disease = strongest level evidence per disease
        disease_logits = evidence_matrix.max(dim=2).values  # [B, 12]
        # Location = strongest disease evidence per level
...
class HVALoss(nn.Module):
    """
    Combined loss for HVA-Net:
    L_total = L_disease + L_loc + λ * L_joint

    L_joint: BCE on [B, 12, 5] evidence matrix
    Forces model to learn disease-location binding.
    """
    ...
    def forward(self, disease_logits, loc_logits, evidence_matrix,
        ...
        # Joint evidence loss — use paired samples only
...
def train_epoch(model, loader, optimizer, criterion):
    model.train()
    model.bert.eval()     # BERT frozen → keep deterministic
    model.siglip2.eval()  # SigLIP2 frozen → keep deterministic
    total_loss  = 0.0
    total_joint = 0.0
    correct_cls = 0
    total_cls   = 0
...
def main():
    print(f"\n{'='*65}")
    print("E7a — HVA-Net: Hierarchical Vertebra-Aware Network")
    print("Novel: Disease-Location Evidence Matrix [12×5]")
    print(f"{'='*65}\n")

    wandb.init(
        project="SpineVQA-CL",
        name="E7a-HVANet-EvidenceMatrix",
        config={
            "model":         "E7a-HVANet",
            "description":   "VertAttn + DiseaseQuery + Evidence Matrix",
            ...
            "novel_modules": "DiseaseQueryAttn + EvidenceMatrix + JointLoss",
        }
    )
    ...
    print(f"\nNovel modules:")
    print(f"  VertebralAttention:    {vert_p:,} params")
    print(f"  DiseaseQueryAttention: {disease_p:,} params")
    print(f"  EvidenceMatrix:        computed (no extra params)")
    ...
    for epoch in range(1, cfg.EPOCHS + 1):
        ...
        if val_metrics["overall_acc"] > best_val_overall:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(cfg.SAVE_DIR, "E7a_best.pth")
            )
            saved = "✓"
        else:
            saved = ""
    ...
    print(f"\nE7a HVA-Net FINAL TEST RESULTS:")
    ...
    print(f"  E7a (evidence matrix joint):  "
          f"{test_metrics['overall_acc']:.2f}%  "
          f"({'↑' if diff > 0 else '↓'}{abs(diff):.2f}%)")

    wandb.finish()
    print(f"\nModel: {cfg.SAVE_DIR}/E7a_best.pth")
